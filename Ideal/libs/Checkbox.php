 ...
 * @license GNU General Public License 2.0
 */
class Checkbox_ideal extends Typecho_Widget_Helper_Form_Element
{
    ...
    public function __construct($name = NULL, array $options = NULL, $value = NULL, $label = NULL, $description = NULL)
    {
        ...
        $this->addItem(new Ideal('<div class="mdui-panel" mdui-panel=""><div class="mdui-panel-item"><div class="mdui-panel-item-header">'.$label. '</div><div class="mdui-panel-item-body"><ul class="typecho-option" id="typecho-option-item-'.$name.'-'.self::$uniqueId.'">'));
    ...
    public function input($name = NULL, array $options = NULL)
    {
        foreach ($options as $value => $label) {
            ...
            */
            ...
            $item->addItem(new Ideal('<label class="mdui-checkbox">'));
            $item->addItem($this->_options[$value]->setAttribute('name', $this->name . '[]')
                ->setAttribute('type', 'Checkbox')
                ->setAttribute('value', $value)
                ->setAttribute('id', $id));
            $item->addItem(new Ideal("<i class=\"mdui-checkbox-icon\"></i>
$label</label>"));

            /*$labelItem = new Ideal('<label class="mdui-checkbox"><input name="'.$this->name.'[]" type="Checkbox" value="'.$value.'" id="'.$id.'"/><i
